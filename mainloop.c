...
void MainLoop(void)
{
	...
	for (;;)
	{
		...
		if (Adc_Get(&pot, &iout, &uin))				// новые значения от АЦП?
		{
		...
		if (MainTimer_Tick())
		{
			...
			if (++cnt >= 2)
			{
				...
				Motor_Enable((properties.uin > UIN_MIN) /*&& (properties.uin < UIN_MAX)*/ && !Interface_IsFreeMode());	// fix: защита по большому напряжению убрана
			}
		}
	}
}
...
// расчёт текущей скорости (относительных единиц за 0.02 секунды)
...
// обновление текущей команды с учётом скорости
